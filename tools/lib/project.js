...
function projectStep(spec, st, unknown, where, box) {
  ...
  /* ⛔ THE BRANCH FLAG COMES FROM THE EMITTED BLOCK, NOT FROM THE SPEC. The
     ...
     still counts where it exists. Since 2026-09-22 the block is the SHELF block,
     so a shelf-only guide gets its derived flag by the same mechanism. */
 ...
 *  SHELF BLOCK — the app's emitter over every spec (ruling 2026-09-22). Absent
 *  means the block is stale for this row; publish.js refuses rather than guesses.
 */
...
/* ⛔ TWO SOURCES, ON PURPOSE (ruling 2026-09-22). SOURCED_FOR — the row -> guide map the
   splice emits — is read from the SHELF BLOCK, because that is the emit over every spec and
   the page carries only the app's subset; a shelf-only guide has its route there and nowhere
   else. TEXT_GUIDE_FAM is the app's own family routing and stays on the page. */
function parseRouting(html, shelfBlock) {
  if (typeof shelfBlock !== 'string' || !shelfBlock.length) {
    throw new Error('⛔ REFUSING — parseRouting needs the shelf block; routing is no longer read off the page alone');
  }
  const obj = (decl, required, text, where) => {
    const at = text.indexOf(decl);
    if (at < 0) {
      if (required) throw new Error('⛔ REFUSING — "' + decl + '" not found in ' + where);
      return {};
    }
    const html = text;
  ...
  const sourcedFor = obj('const SOURCED_FOR = {', true, shelfBlock, 'scratchpad/clean-rebuild/shelf_block.js');
  const textGuideFam = obj('const TEXT_GUIDE_FAM = {', true, html, 'www/index.html');
