...
git -C ../fieldstrip-library add guides index.json changelog.json   # the publish output, by name
git -C ../fieldstrip-library status --porcelain                     # READ IT — anything else is not yours
git -C ../fieldstrip-library commit -m "publish vN"
git -C ../fieldstrip-library push
```

⛔ **`git add -A` IS NO LONGER DESCRIBED AS SAFE HERE, AND THE LINE THAT SAID SO
IS STRUCK** (Darren ruling 2026-08-22). It read *"`git add -A` is safe in the
library repo only"*, and that rested on an assumption nobody wrote down: **one
writer.** On 2026-08-22 two lanes were in this tree at once — a guide lane and a
public-domain plate lane — and `-A` would have swept an unfinished plate hunt
into a publish and onto a **public** site. Neither lane would have seen it,
because `-A` is exactly the command that stops you looking.

**Stage the publish output by name and read `git status --porcelain` before you
commit.** If a path you did not produce appears, it belongs to someone else's
lane: leave it, and say so. The rule is now identical in both repositories,
which is the point — a rule with an exception is a rule people misremember.

⚠ **And `plates/` is generated by the python lane, not by `publish.js`** — the
publisher reads it and refuses anything that did not come through that lane. It
is staged by whoever produced it, never as a side effect of a guide publish.
